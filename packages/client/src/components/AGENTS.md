...
| `SessionGraph.tsx` | Workspace-grouped fork tree view (replaces list when SessionViewToggle=graph). Per-node status dot + label + status text + trash button (`deleteSession` REST → `session_deleted` broadcast drops the card). See change: add-session-delete-button. |
