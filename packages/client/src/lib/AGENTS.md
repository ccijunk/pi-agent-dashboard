...
| `session-control-api.ts` | REST wrappers for per-session control ops not on the WS bridge. Exports `deleteSession(sessionId)` — POST `/api/session/:id/delete` (hard delete); server broadcasts `session_deleted`. See change: add-session-delete-button. |
